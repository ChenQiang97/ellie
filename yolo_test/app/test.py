...
if __name__ == '__main__':
    img_dir = r"D:\workspace\yolo_test\ultralytics-main\ultralytics\assets"
    # Load a model
    # 直接使用预训练模型创建模型
    ...
    #     # 推理：
    # model = YOLO('yolov8n.pt')
    # model.predict(source=img_dir, show=True, save=True)
    # model.predict(source='ultralytics/assets', name='predict', **{'save':True})   # 写法二

    # 分割：
    model = YOLO('yolov8n-seg.pt')
    model.predict(source=img_dir, name='predict', **{'save': True})
    ...
    # model.track(source=img_dir, show=True, save=True)
